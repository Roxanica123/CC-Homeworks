...
import { AuthenticationService, JwtService, UserService } from "./services";
...
app.post(
  ...
  async (req: any, res: any) => {
    ...
    const response: HttpActionResult =
      await new AuthenticationService().registerUser(req.body);
    res.statusCode = response.statusCode;
...
app.get("/profile/:userEmail", async (req, res) => {
  let hasAccess = await RequestsValidationService.instance.hasAcces(req, [
    UserTypes.ADMIN,
    UserTypes.BASIC,
    UserTypes.MODERATOR,
    UserTypes.PREMIUM,
  ...
  const response: HttpActionResult =
    hasAccess === true
      ? await new UserService().getProfileByEmail(email)
      : hasAccess;
  res.statusCode = response.statusCode;
  res.end(response.body);
});
...
app.patch("/premium", async (req, res) => {
  ...
  const response: HttpActionResult =
    hasAccess === true
      ? await new AuthenticationService().changeUserRole(
          req.body.email,
          UserTypes.PREMIUM
        )
      : hasAccess;
